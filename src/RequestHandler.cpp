...
void	RequestHandler::handleDeleteRequest(Client &client, HttpResponse &response, RequestHandlerConf &config)
{
	(void)client;
	(void)config;
	(void)response;
...
void	RequestHandler::handleInvalidRequest(Client &client, HttpResponse &response, RequestHandlerConf &config)
{
...
void	RequestHandler::processRequest(Client &client)
{
	...
	void	(RequestHandler::*handleMethod[])(Client &, HttpResponse &, RequestHandlerConf &) = {
	...
	for (size_t i = 0; i < _poll.size(); i++) {
		if (_poll[i].fd == client.getSocket()) {
...
void	RequestHandler::handleFileRead(Client &client)
{
...
void	RequestHandler::handleDirectoryRequest(Client &client, HttpRequest &request, RequestHandlerConf &config)
{
	//*******get correct conf from RequestHandlerConf / LocationConf*****
	std::string path = getPathFromUrl(client.getRequest().getPath(), config);
	//Check for index files first
	//somehow retreive location conf -> RequestHandlerConf should have some public function getLocationConf(std::string path)
	...
	if (_config.getAutoIndex()) { //CHECK IF THIS IS REALLY THE PARAM WE NEED TO CHECK IF DIRECTORY LISTING IS ENABLED!?!?! -> should be different depending on the location or all the RequestHandler has the same!?!?
...
void	RequestHandler::handleDirectoryListing(Client &client, HttpRequest &request, RequestHandlerConf &config)
{
...
void	RequestHandler::handleGetRequest(Client &client, HttpResponse &response, RequestHandlerConf &config)
{
...
void	RequestHandler::handlePostRequest(Client &client, HttpResponse &response, RequestHandlerConf &config)
{
	(void)client;
	(void)config;
	(void)response;
